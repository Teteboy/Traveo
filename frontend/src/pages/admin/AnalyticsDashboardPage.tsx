      ...
      {!loading && !hasError && (
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <Card className="border-slate-200"><CardContent className="p-4"><div className="flex items-start justify-between"><div><p className="text-sm text-slate-500 mb-1">Revenus Totaux</p><p className="text-2xl font-bold text-slate-900">{(overviewStats.totalRevenue / 1000).toFixed(0)}K FCFA</p></div><div className={'flex items-center gap-1 text-sm ' + (overviewStats.revenueGrowth >= 0 ? 'text-green-600' : 'text-red-600')}>{overviewStats.revenueGrowth >= 0 ? <ArrowUpRight className="h-4 w-4" /> : <ArrowDownRight className="h-4 w-4" />}{Math.abs(overviewStats.revenueGrowth)}%</div></div><div className="mt-3 flex items-center gap-2"><Banknote className="h-4 w-4 text-slate-400" /><span className="text-xs text-slate-500">vs. période précédente</span></div></CardContent></Card>
              ...
              <CardContent className="space-y-4">
                {recentActivity.map((activity) => (
                  <div key={activity.id} className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0"><p className="text-sm text-slate-900 truncate">{activity.description}</p><div className="flex items-center justify-between mt-1"><span className="text-xs text-slate-500">{activity.time}</span>{activity.amount > 0 && <span className="text-sm font-medium text-green-600">+{activity.amount} FCFA</span>}</div></div>
                  </div>
                ))}
              </CardContent>
            </Card>
