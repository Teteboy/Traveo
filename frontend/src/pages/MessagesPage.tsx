  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const preselectedId = params.get('conversationId')

        if (preselectedId && data.some((t: any) => t.id === preselectedId)) {
          setSelectedThread(preselectedId)
        ...
        <Card className="lg:col-span-1 flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto">
            {loading ? (
              ...
              <div className="p-4 text-sm text-slate-500">Aucune conversation pour le moment.</div>
            ) : (
              threads.map(thread => (
                <button
                  key={thread.id}
