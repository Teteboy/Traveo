...
async function main() {
  ...
  const videos = [
    ...
    { userId: provider2User.id, title: 'Mont Cameroun - L\'ascension', videoUrl: 'https://www.youtube.com/embed/9bZkp7q19f0', thumbnailUrl: 'https://images.unsplash.com/photo-1464822759023-fed622ff2c3b?w=400', destination: 'Buea', country: 'Cameroun', likes: 2100, views: 78000 },
  ...
  for (const v of videos) {
    ...
    if (existing) {
      await prisma.video.update({ where: { id: existing.id }, data: { userId: v.userId } })
    } else {
